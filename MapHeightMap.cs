...
#pragma warning disable CS8602 // Dereference of a possibly null reference.
            ...
            XDocument mapHeightMapDoc = XDocument.Parse(content);


            IEnumerable<XElement?> mapImageBitmapElem = mapHeightMapDoc.Descendants().Select(x => x.Element(ns + "Bitmap"));
            if (mapImageBitmapElem != null && mapImageBitmapElem.Any() && mapImageBitmapElem.First() != null)
            {
                string? base64String = mapHeightMapDoc.Descendants().Select(x => x.Element(ns + "Bitmap").Value).FirstOrDefault();
